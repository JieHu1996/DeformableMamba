...
class HFBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.blocks.append(norm_layer(hidden_dim))
        self.blocks.append(SS2D(d_model=hidden_dim, 
                            d_state=ssm_d_state, 
                            ssm_ratio=ssm_ratio,
                            dt_rank=ssm_dt_rank,
        ...
        if fusion_mode == 'conv':
            self.fusion = ConvBlock(in_chans=hidden_dim*2,
            ...
            self.fusion = Mlp(in_features=hidden_dim*2, 
    ...
    def forward(self, x_deep, x_shallow):
        ...
        for block in self.blocks:
            x = block(x)
        x_deep = x_deep + self.norm1(x)
        x = torch.cat([x_deep, x_shallow], dim=1)
        x = self.drop_path(self.fusion(self.norm2(x)))
        return x
...
class SegMambaHead(BaseDecodeHead):
    def __init__(
        self,  
        depths=[2, 2, 2, 2], 
