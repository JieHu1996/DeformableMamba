...
class DFT(nn.Module):
    ...
    def __init__(self, in_chans=768, out_chans=768, kernel_size=3, stride=1, padding=1, norm_layer=nn.BatchNorm2d):
        ...
        self.offset_conv = nn.Conv2d(in_channels=in_chans, out_channels=2*kernel_size*kernel_size, 
                                     kernel_size=kernel_size, stride=stride, padding=padding)
        self.modulator_conv = nn.Conv2d(in_channels=in_chans, out_channels=kernel_size * kernel_size, 
                                        kernel_size=kernel_size, stride=stride, padding=padding)
    ...
    def _init_weights(self):
        constant_init(self.offset_conv.weight, 0.)
        constant_init(self.offset_conv.bias, 0.)
        constant_init(self.modulator_conv.weight, 0.)
        constant_init(self.modulator_conv.bias, 0.)
...
class HFBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.if_fusion:
            ...
            if fusion_mode == 'conv':
                self.fusion = ConvBlock(in_chans=dim,
                                        out_chans=dim,
                ...
                hidden_features = int(dim * fusion_scale_ratio)
                self.fusion = Mlp(in_features=dim, 
                                hidden_features=hidden_features, 
                                out_features=dim,
                                act_layer=fusion_act_layer, 
                                drop=fusion_drop_rate)
            elif fusion_mode == 'dconv':
                self.fusion = DFT(in_chans=dim, out_chans=dim, norm_layer=norm_layer)
            else:
                raise NotImplementedError("Error: only 'conv', 'mlp' and 'dconv' are accepted as valid parameters.")
    ...
    def forward(self, x_deep, x_shallow):
        ...
        if self.if_fusion:
            x = self.fusion(x_sum)
            x = self.ssm(x)
            x_sum = x_sum + self.drop_path(x)
        x = torch.cat([x_sum, x_shallow], dim=1)
...
class SegMambaHeadV7(BaseDecodeHead):
    def __init__(
        self,  
        depths=[2, 2, 2, 2], 
