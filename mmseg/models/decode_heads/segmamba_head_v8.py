...
from ..utils.mamba2 import Mlp, LayerNorm2d, InitMamba, cross_scan_fn, cross_merge_fn, selective_scan_fn
...
class HFBlock(nn.Module):
    ...
    def forward(self, x_deep, x_shallow):
        ...
        x = self.local_branch(x_deep) + self.global_branch(x_deep)
        x_deep = x_deep + self.drop_path(x)
...
class SegMambaHeadV8(BaseDecodeHead):
    def __init__(
        self,  
        depths=[2, 2, 2, 2], 
