...
class CGRSeg(BaseDecodeHead):
    def __init__(self, is_dw=False, next_repeat=4, mr=2, dw_size=7, neck_size=3, square_kernel_size=1, module='RCA', ratio=1, **kwargs):
        ...
        # we use different version of mmengine, which do not support passing norm_cfg and act_cfg in conv2d. 
        # Therefore, we define these two functions ourselves
    ...
    def forward(self, inputs):
        ...
        for i in range(len(self.in_channels)-1,-1,-1):
            ...
            results.append(flag)
        x = results[-1]
        _c = self.act(self.bn(self.linear_fuse(x)))
