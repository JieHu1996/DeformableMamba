...
class HFBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.if_up_sample:
            self.channnel_mix = Mlp(
                in_features=dim // 2, 
                hidden_features=int(dim // 2 * local_scale_ratio), 
                out_features=dim // 2,
                act_layer=local_act_layer, 
                drop=local_drop_rate
                )
    ...
    def forward(self, fmap_dp, fmap_sh):
        ...
        """
        fmap_sum = fmap_dp + fmap_sh
        attn = self.act(self.attn(fmap_sum))
        ...
        if self.if_up_sample:
            x = self.upsampling(x)
            # x = self._up_sample(x)
            x = x + self.drop_path(self.channnel_mix(x))
        return x
...
class SegMambaHeadV15(BaseDecodeHead):
    def __init__(
        self,  
        depths=[2, 2, 2, 2], 
