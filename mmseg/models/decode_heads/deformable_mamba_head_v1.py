...
class DFT(nn.Module):
    ...
    def transform(self, x):
        max_offset = torch.tensor((min(x.shape[-2], x.shape[-1]) // 4), dtype=x.dtype).to(x.device)
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, 
                 ...
                 layer_scale=None,):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channel, skip_channels, kernel_size=kernel_size, stride=1)
        self.norm1 = norm_layer(skip_channels, eps=1e-5)
        self.act = act_layer()
        self.conv2 = nn.Conv2d(skip_channels, out_channel, kernel_size=kernel_size, stride=1)
        self.norm2 = norm_layer(out_channel, eps=1e-5)
        # self.layer_scale = layer_scale
        # if layer_scale is not None and type(layer_scale) in [int, float]:
        #     self.gamma = nn.Parameter(layer_scale * torch.ones(out_channel))
...
class HFBlock(nn.Module):
    ...
    def forward(self, x_deep, x_shallow):
        ...
        if self.if_up_sample:
            ...
            h, w = x.shape[-2:]
            target_size = (h * 2, w * 2)
            x = F.interpolate(x, size=target_size, mode=self.interpolate_mode, align_corners=False)
        else:
            x = self.conv(x)
        return x
...
class DeformableMambaHeadV1(BaseDecodeHead):
    def __init__(
        self,  
        depths=[2, 2, 2, 2], 
