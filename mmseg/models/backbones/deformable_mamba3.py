...
from ..utils.mamba2 import LayerNorm2d, create_patch_embed, create_block
...
def create_down_sample(dim):
    return nn.Sequential(
        nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0),
        nn.SyncBatchNorm(dim),
        nn.PixelUnshuffle(2),
        nn.Conv2d(dim * 4, dim * 2, kernel_size=1, stride=1, padding=0),
...
class DeformableMAMBAv3(BaseModule):
    def __init__(
            self, 
            patch_size=4, 
            in_chans=3, 
        ...
        for i in range(len(depths)):
            downsample = create_down_sample(self.dims[i]) if (i < len(depths) - 1) else nn.Identity()
